...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='config/sign/train/train_joint_300.yaml',
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        arg.model_saved_name = f"./save_models/{arg.Experiment_name}_class{arg.model_args['num_class']}/{arg.Experiment_name}"
        os.makedirs(f"./save_models/{arg.Experiment_name}_class{arg.model_args['num_class']}", exist_ok=True)
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            acc = torch.mean((predict_label == loss.item()).float())
        ...
        with torch.no_grad():
            ...
            for ln in loader_name:
                ...
                for batch_idx, (data, label, index) in enumerate(process):
                    ...
                    with open('output_bone_300.csv', 'a+') as out:
                        wt = csv.writer(out, lineterminator='\n')
                        for result in output:
                            wt.writerow((x.item() for x in result))
