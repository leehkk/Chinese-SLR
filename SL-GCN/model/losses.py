...
class SPLLoss(nn.Module):
    def __init__(self, n_samples=0,threshold = 0.1,growing_factor = 1.3):
...
val_interval = 5
save_interval = 5
path_to_csl = "C:/uestc/code/TimeSformer-main/datasets/{}-{}".format(dataset, num_classes)
...
if __name__ == '__main__':
    if phase == "Train":
        train_set = AUSLR(path_label_dir=path_to_csl, frames=num_frames,
            num_classes=num_classes, split="train")
        val_set = AUSLR(path_label_dir=path_to_csl, frames=num_frames,
            num_classes=num_classes, split="val")
        logger.info("AUSLR Train samples: {}".format(len(train_set)))
        logger.info("AUSLR Val samples: {}".format(len(val_set)))
        train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True)
        val_loader = DataLoader(val_set, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
    else:
        test_set = AUSLR(path_label_dir=path_to_csl, frames=num_frames,
                        num_classes=num_classes, split="test")
        logger.info("AUSLR Test samples: {}".format(len(test_set)))
    ...
    if phase == 'Train':
        ...
        for epoch in range(epochs):
            ...
            # Train the model
            criterion.init_v()
            train_epoch(model, criterion, optimizer, train_loader, device, epoch, logger, log_interval, writer,batch_size)
        ...
        weight_path = os.path.join(result_path,"swin_epoch145.pth")
